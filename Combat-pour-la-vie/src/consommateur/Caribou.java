...
public class Caribou extends Cervide{
	...
	public Caribou(Sexe s, int v, Case c, int cmc, int a, int ddv, int fc) {
	...
	public Consommateur recherche_reproducteur() {
		...
		if(l!=null) {
			...
			while(it.hasNext()) {
				...
				if(c_test instanceof consommateur.Caribou ) {
    	...
    	if(nom_classe_this.equals(nom_classe_c) && this.getSexe() != c.getSexe()) {
    		...
    		if(new_emplacement!=null) {
	    		...
	    		if(Math_methods.randomWithRange(0,1) == 1) 
	    			es = Sexe.male;
	    		Caribou l = new Caribou(es, Math_methods.randomWithRange(this.getVie()-1, c.getVie()+1),
    				new_emplacement, 
